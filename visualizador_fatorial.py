...
# ═══════════════════════════════════════════════════════════
ETAPA1_LINHAS = [
    ("1", "fatorial(4)", "4 × ?",   "#A5B4FC"),
    ("2", "fatorial(3)", "3 × ?",   "#A5B4FC"),
    ("3", "fatorial(2)", "2 × ?",   "#A5B4FC"),
    ("4", "fatorial(1)", "1 × ?",   "#A5B4FC"),
    ("5", "fatorial(0)", "retorna 1","#86EFAC"),
]

ETAPA1_COMENTARIOS = [
    "fatorial(4) é chamada primeiro — ela precisa do resultado de fatorial(3) para continuar, então fica pausada na pilha.",
    "fatorial(3) é empilhada — também não pode terminar sozinha, precisa de fatorial(2). Fica pausada.",
    "fatorial(2) entra na pilha — pausa e espera o resultado de fatorial(1).",
    "fatorial(1) é empilhada — pausa e aguarda fatorial(0).",
    "fatorial(0) é o CASO BASE: não faz chamada recursiva. Retorna 1 imediatamente. A recursão para aqui!",
]

ETAPA2_PASSOS = [
    (3, "1", "fatorial(1) recebe 1 de volta  →  calcula 1 × 1 = 1  →  desempilha."),
    (2, "1", "fatorial(2) recebe 1 de volta  →  calcula 2 × 1 = 2  →  desempilha."),
    (1, "2", "fatorial(3) recebe 2 de volta  →  calcula 3 × 2 = 6  →  desempilha."),
    (0, "6", "fatorial(4) recebe 6 de volta  →  calcula 4 × 6 = 24  →  pilha vazia! ✓"),
]
...
class App:
    ...
    def _avancar_e1(self):
        ...
        if i > 0:
            prev_cells, prev_bg = self.row_frames[i - 1]
            for c in prev_cells:
                c.configure(bg=prev_bg)
                for w in c.winfo_children():
                    self._set_bg_tree(w, prev_bg)
            lv_prev = self.val_labels[i - 1]
            if lv_prev:
    ...
    def _show_etapa2(self):
        ...
        tk.Label(frm_top, text="Preenchendo os retornos",
                 font=("Georgia", 22, "bold"), bg=BG, fg=HEADER_FG).pack(anchor="w")
        tk.Label(frm_top,
                 text="A pilha desempilha de baixo para cima — cada ? recebe o valor calculado.",
                 font=("Helvetica", 12), bg=BG, fg=MUTED_FG).pack(anchor="w", pady=(4, 0))
